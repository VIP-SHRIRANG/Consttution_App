...
app.post('/search-json', (req, res) => {
    ...
    fs.readFile(jsonFilePath, 'utf8', (err, data) => {
        ...
        if (results.length === 0) {
            results.push({ title: 'No results found', description: 'Please try a different search term.' });
        }
...
app.post('/search-scrape', async (req, res) => {
    ...
    const url = `https://www.indiankanoon.org/search/?formInput=${encodeURIComponent(query)}`;
    try {
        ...
        $('.result_title').each((i, element) => {
            ...
            const link = `https://www.indiankanoon.org${$(element).find('a').attr('href')}`;
...
app.post('/chatbot', async (req, res) => {
    ...
    const responseFilePath = path.join(__dirname, 'data', 'chatbot_responses.json');
    ...
    fs.readFile(responseFilePath, 'utf8', (err, data) => {
        ...
        if (!response) {
            fs.readFile(lawFilePath, 'utf8', (err, data) => {
                ...
                const laws = JSON.parse(data);
                response = "Sorry, I couldn't find any relevant information. Can you try asking differently?";
